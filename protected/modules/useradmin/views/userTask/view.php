...
$this->breadcrumbs = array (
		Yii::t("UserAdminModule.breadcrumbs", "Manage user tasks") => array (
				'admin' 
		),
...
echo CHtml::link(Yii::t("UserAdminModule.admin", "Manage"), array (
										"admin" 
								), array (
...
echo CHtml::link(Yii::t("UserAdminModule.admin", "Edit"), array (
										"update",
										"id" => $taskDetails->id 
								), array (
										"class" => "btn btn-small pull-right" 
...
$this->widget('zii.widgets.CDetailView', array (
		...
		'htmlOptions' => array (
				'class' => 'table table-condensed table-striped table-hover table-bordered table-side' 
		) 
));
?>
...
echo CHtml::htmlButton('<i class="icon-ok icon-white"></i> ' . Yii::t("UserAdminModule.admin", "Save"), array (
...
echo CHtml::link(Yii::t("UserAdminModule.admin", "Refresh controllers list"), array (
																		'refreshOperations',
																		'id' => $taskDetails->id 
																), array (
...
<table class='table table-bordered table-condensed'>
	...
	<tr>
		<td class='controllers-box'>
			<table class='table'>
				<tr>
					<td width='50%'><span class='btn btn-block btn-small commonHider'>
                                                                <?php echo Yii::t("UserAdminModule.admin","Hide all"); ?>
                                                        </span></td>
					<td width='50%'><span
						class='btn btn-block btn-small btn-success commonOpener'>
                                ...
                                <?php foreach($generalControllers as $generalController): ?>
                                        <?php
																																	...
																																	if (! isset($prevName) or ($prevName != $controllerName)) {
																																		...
																																		echo "<div class='controllerNameContainer'>" . " <span class='commonControllerName' param='{$controllerName}'><i class='icon-plus'></i> {$controllerName}</span></div>";
																																	}
																																	?>

                                        <label
			class="checkbox hide commonControllerHide commonController<?php echo $controllerName; ?>">
                                                <?php
																																	
echo CHtml::checkBox('Operation[' . $generalController->id . ']', in_array($generalController->id, $operationIds), array (
                                ...
                                <?php foreach($moduleControlers as $moduleControler): ?>
                                        <?php
																																	...
																																	if (! isset($prevModuleName) or ($prevModuleName != $moduleName)) {
																																		...
																																		echo "<h3>{$moduleName} <span class='btn btn-small moduleHider' param='{$moduleName}'>" . Yii::t("UserAdminModule.admin", "Hide all") . "</span> <span class='btn btn-small btn-success moduleOpener' param='{$moduleName}'>" . Yii::t("UserAdminModule.admin", "Open all") . "</span></h3>";
																																	}
                                        ...
                                        <label
					class='checkbox hide module<?php echo $moduleName; ?> moduleController<?php echo $moduleName.'-'.$controllerName; ?>'>
                                                <?php
																																	
echo CHtml::checkBox('Operation[' . $moduleControler->id . ']', in_array($moduleControler->id, $operationIds), array (
