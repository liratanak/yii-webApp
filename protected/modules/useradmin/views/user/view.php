...
$this->breadcrumbs = array (
		Yii::t("UserAdminModule.breadcrumbs", "Manage users") => array (
				'admin' 
		),
	...
	<tr>
		<td style='padding: 0 10px;'>
                        <?php if(Yii::app()->user->getFlash('roleSaved')): ?>
                                <h4
				class='alert alert-success centered hide-on-click'>
...
echo $roleForm->checkBoxList($model, 'roleIds', CHtml::listData(UserRole::model()->findAll(), 'code', 'name'), array (
...
echo $taskForm->checkBoxList($model, 'taskIds', CHtml::listData(UserTask::model()->findAll("code != 'freeAccess'"), 'code', 'name'), array (
																																		'template' => "<label class='checkbox'>{input} {label}</label>",
																																		'separator' => '' 
																																));
																																?>
