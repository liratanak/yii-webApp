...
class UAccessController extends CController {
	...
	private function _getAllowedUserRoutes() {
		...
		} else 		// If no cached routes
		{
			...
			$roleRoutes = Yii::app()->db->createCommand($sqlRoles)->bindParam(':user_id', $userId, PDO::PARAM_INT)->queryAll();
			
			$taskRoutes = Yii::app()->db->createCommand($sqlTasks)->bindParam(':user_id', $userId, PDO::PARAM_INT)->queryAll();
